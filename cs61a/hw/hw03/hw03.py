...
def count_dollars(total):
    ...
    def helper(amount, bill):
        # 终止条件,相当于四种情况amount > < == 0，还有就是bill is None
        if amount == 0: # 纸币刚好用完
            return 1
        if amount < 0: # 凑过头了
            return 0
        if bill is None: # 纸币用完了还没凑出来
            return 0
        next_bill = next_smaller_dollar(bill)
        # 这里是核心
        use_bill = helper(amount - bill, bill)# 至少使用1张当前美元
        skip_bill = helper(amount, next_bill)# 不使用当前美元
