...
def boar_brawl(player_score, opponent_score):
    ...
    "*** YOUR CODE HERE ***"
    # END PROBLEM 2
    playerones_digit = player_score % 10
    opponenttens_digit = opponent_score // 10 % 10
    sumpoint = 3 * abs(playerones_digit - opponenttens_digit)
    if sumpoint == 0:
        sumpoint = 1
    return sumpoint
...
def take_turn(num_rolls, player_score, opponent_score, dice=six_sided):
    ...
    # END PROBLEM 3
    if num_rolls !=0:
        sumpoint = roll_dice(num_rolls,dice=dice)
    else:
        sumpoint = boar_brawl(player_score, opponent_score)
    return sumpoint
...
def always_roll(n):
    ...
    # BEGIN PROBLEM 6
    # 高阶函数,这里always_roll()最后返回的是strategy函数,并不是一个特定的值
    "*** YOUR CODE HERE ***"
...
def is_always_roll(strategy, goal=GOAL):
    ...
    for player_score in range(goal):
        for opponent_score in range(goal):
            if strategy(player_score, opponent_score) != reference:
                return False
    return True
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    max_avg = -1  # Initialize to a value lower than any possible average
    best_num = 1  # Default to 1 if all averages are equal

    for num_rolls in range(1, 11):  # Test 1 to 10 rolls
        # Create a function that averages roll_dice results
        averaged_roll = make_averaged(roll_dice, times_called)
        # Calculate average score for this number of rolls
        ...
        if current_avg > max_avg:
            max_avg = current_avg
            best_num = num_rolls
        # If tie, keep the smaller number
        elif current_avg == max_avg and num_rolls < best_num:
            best_num = num_rolls
    return best_num
    # END PROBLEM 9
...
def boar_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 10
    newscore = boar_brawl(score, opponent_score)
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    Suss Fuss rules."""
    # BEGIN PROBLEM 11
    newscore = sus_update(0,score, opponent_score, threshold)
